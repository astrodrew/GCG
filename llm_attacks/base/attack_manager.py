...
class MultiPromptAttack(object):
    ...
    def get_filtered_cands(self, worker_index, control_cand, filter_cand=True, curr_control=None):
        ...
        for i in range(control_cand.shape[0]):
            if worker.conv_template.name == 'llama-3':
                # decoded_str = worker.tokenizer.decode(control_cand[i], skip_special_tokens=True)
                decoded_tokens = worker.tokenizer.convert_ids_to_tokens(control_cand[i])
                decoded_str = worker.tokenizer.convert_tokens_to_string(decoded_tokens)
                if filter_cand:
                    if decoded_str != curr_control  and len(control_cand[i]) <= 20:
        ...
        if len(cands) == 0:
            ...
            length_list = []
            for t in temp:
                length_list.append(abs(len(worker.tokenizer(t, add_special_tokens=False).input_ids)-20))
            min_value = min(length_list)
            min_index = length_list.index(min_value)
